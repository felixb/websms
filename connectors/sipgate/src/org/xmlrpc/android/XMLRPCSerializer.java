...
class XMLRPCSerializer implements IXMLRPCSerializer {
	...
	public void serialize(final XmlSerializer serializer, final Object object)
		...
		if (object instanceof Integer || object instanceof Short
			...
			serializer.startTag(null, TYPE_I4).text(object.toString()).endTag(
					null, TYPE_I4);
		} else if (object instanceof Long) {
			serializer.startTag(null, TYPE_I8).text(object.toString()).endTag(
					null, TYPE_I8);
			...
			serializer.startTag(null, TYPE_BOOLEAN).text(boolStr).endTag(null,
					TYPE_BOOLEAN);
			...
			serializer.startTag(null, TYPE_BASE64).text(value).endTag(null,
					TYPE_BASE64);
			...
			while (iter.hasNext()) {
				...
				serializer.startTag(null, TAG_NAME).text(key).endTag(null,
						TAG_NAME);
				serializer.startTag(null, TAG_VALUE);
				this.serialize(serializer, value);
				serializer.endTag(null, TAG_VALUE);
			...
			this.serialize(serializer, serializable.getSerializable());
		} else {
			throw new IOException("Cannot serialize " + object);
		}
	}

	public Object deserialize(final XmlPullParser parser)
		...
		if (hasType) {
			if (typeNodeName.equals(TYPE_INT) || typeNodeName.equals(TYPE_I4)) {
				...
				while (parser.getName().equals(TAG_VALUE)) {
					list.add(this.deserialize(parser));
					parser.nextTag();
				}
				...
				while (parser.getName().equals(TAG_MEMBER)) {
					...
					while (true) {
						...
						if (name.equals(TAG_NAME)) {
							...
							memberValue = this.deserialize(parser);
						} else {
							break;
						}
					}
